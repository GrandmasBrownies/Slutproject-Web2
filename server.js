...
app.get('/', (req, res) => {
 ...
 app.post('/post', upload.single('img'), function(req,res){
    ...
    const sqlInstert = "INSERT INTO tabellnamn (title, ingredients, instructions, image ) VALUES (?, ?, ?, ?);"
 ...
 app.post('/addcart', function(req, res) {
    ...
    db.query('SELECT * FROM tabellnamn WHERE title = ? AND ingredients = ?', [title, ingredients], (err, results) => {
        if (err) {
            throw err;
        } else {
            const cart = results;
